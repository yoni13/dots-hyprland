...
2. Run `uv pip compile requirements.in --torch-backend rocm6.4 -o requirements.txt` in this folder.

**Notes:**
- For reference see [uv doc](https://docs.astral.sh/uv/pip/dependencies/#using-requirementsin).
- `pyproject.toml` configures the PyTorch backend (ROCm 6.4) for uv, ensuring the correct ROCm wheels are used when compiling or installing dependencies.
